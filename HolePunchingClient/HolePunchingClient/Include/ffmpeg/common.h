...
#    define av_always_inline __attribute__((always_inline)) __inline
#else
...
static __inline int av_log2_16bit(unsigned int v)
{
    int n = 0;
    if (v & 0xff00) {
        v >>= 8;
...
/* median of 3 */
static __inline int mid_pred(int a, int b, int c)
{
#ifdef HAVE_CMOV
    int i=b;
    asm volatile(
 ...
 * @return clipped value
 */
static __inline uint8_t av_clip_uint8(int a)
{
 ...
 * @return clipped value
 */
static __inline int16_t av_clip_int16(int a)
{
 ...
 */
static __inline int ff_get_fourcc(const char *s){
#ifdef HAVE_AV_CONFIG_H
    assert( strlen(s)==4 );
#endif
...
static __inline long long read_time(void)
{
    long long l;
    asm volatile("rdtsc\n\t"
                 : "=A" (l));
